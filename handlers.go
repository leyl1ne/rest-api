...
type User struct {
	...
	Acitve    int    `json:"active"`
}

// encodes a slice with json records
...
// this handler is executed when the HTTP method is incorrect
...
func AddHandler(rw http.ResponseWriter, r *http.Request) {
	...
	if len(d) == 0 {
		rw.WriteHeader(http.StatusBadRequest)
		log.Println("Not input!")
		return
	}

	// We reading two structures as an array
	// 1. User who executes the commands
	// 2. User who was added
...
// GetAllHandler destined which all data custom database
...
func GetIDHandler(rw http.ResponseWriter, r *http.Request) {
	log.Println("GetIDHandler Serving:", r.URL.Path, "form", r.Host)
...
func GetUserDataHandler(rw http.ResponseWriter, r *http.Request) {
	log.Println("GetUserDataHanlder Serving:", r.URL.Path, "from", r.Host)
	id, ok := mux.Vars(r)["id"]
	if !ok {
		log.Println("ID value not set!")
		rw.WriteHeader(http.StatusBadRequest)
...
// LoginHandler is for updating the last login time of a user
// And changing the Active field to true
func LoginHandler(rw http.ResponseWriter, r *http.Request) {
	log.Println("LoginHandler Serving:", r.URL.Path, "form", r.Host)
	d, err := io.ReadAll(r.Body)
	if err != nil {
		rw.WriteHeader(http.StatusBadRequest)
		log.Println(err)
...
func LogoutHandler(rw http.ResponseWriter, r *http.Request) {
	log.Println("LogoutHandler Serving:", r.URL.Path, "form", r.Host)
